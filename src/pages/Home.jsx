...
import { ArrowRight, Star, ShoppingBag, Users, ChevronRight, ChevronLeft, Clock, Calendar, TrendingUp, Heart, Package, Shield, Gift } from 'lucide-react';
...
const ProductCard = ({ product }) => {
    ...
    const handleAdd = (e) => { e.preventDefault(); addToCart(product); setAdded(true); showToast(`${product.name} added to cart!`); setTimeout(() => setAdded(false), 1500); };
    return (
        <Link to={`/product/${product.id}`} className="group block festival-card rounded-2xl overflow-hidden card-3d transition-all duration-300">
            <div className="relative overflow-hidden aspect-square">
                ...
                {product.negotiable && (
                    <div className="absolute top-2 right-2 bg-white/90 rounded-full px-2 py-0.5 text-xs font-bold text-green-600 backdrop-blur-sm">Negotiable</div>
                )}
                {product.mrp > product.price && (
                    <div className="absolute bottom-2 left-2 bg-red-500/90 rounded-full px-2 py-0.5 text-xs font-bold text-white backdrop-blur-sm">
...
const SellerCard = ({ seller }) => (
    <Link to={`/seller/${seller.id}`} className="festival-card rounded-2xl p-5 card-3d transition-all duration-300 block">
        <div className="flex items-center gap-3 mb-3">
            ...
            <div>
                <div className="flex items-center gap-1">
                    ...
                    {seller.verified && <Shield className="w-3.5 h-3.5 text-blue-500 fill-blue-500" />}
                </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-10 items-center">
                        ...
                        <div className="animate-fade-in-up">
                            ...
                            <div className="flex flex-wrap gap-3 mb-8">
                                ...
                                <Link to="/register" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-bold text-sm transition-all hover:scale-105"
                                    style={{ border: '2px solid #9A6B48', color: '#5C4033' }}>
                                    Sell Your Art
                                </Link>
                            </div>
                ...
                <div className="container">
                    ...
                    <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 perspective-container ${visibleSections.has('trending') ? '' : 'opacity-0'}`}>
                        {trendingProducts.map((p, i) => (
                            <div key={p.id} style={{ animation: visibleSections.has('trending') ? `fadeInUp 0.5s ease-out ${i * 0.07}s both` : 'none' }}>
                ...
                <div className="container">
                    ...
                    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 perspective-container ${visibleSections.has('sellers') ? '' : 'opacity-0'}`}>
                        {featuredSellers.map((s, i) => (
                            <div key={s.id} style={{ animation: visibleSections.has('sellers') ? `fadeInUp 0.5s ease-out ${i * 0.1}s both` : 'none' }}>
                                <SellerCard seller={s} />
                            </div>
                        ))}
                    </div>
                ...
                <div className="container">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 perspective-container">
                        {[
                            ...
                            <div key={hamper.name}
                                className="rounded-2xl p-6 card-3d transition-all duration-300 relative overflow-hidden group"
                                style={{
                                    ...
                                    opacity: visibleSections.has('hampers') ? undefined : 0
                                }}>
                ...
                <div className="container">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {recentPosts.map((post, i) => (
                            <div key={post.id} className="festival-card rounded-2xl overflow-hidden card-3d group"
                                style={{
