...
import { Plus, Edit3, Trash2, Package, TrendingUp, Star, DollarSign, X, Check, Eye } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useData } from '../context/DataContext';
import { useNotifications } from '../context/NotificationContext';

const CATEGORIES = ['Handicrafts', 'Food & Sweets', 'Clothing', 'Decoration', 'Jewelry', 'Candles & Diyas', 'Pottery'];
const FESTIVALS_LIST = ['Diwali', 'Holi', 'Navratri', 'Eid', 'Christmas', 'Pongal', 'Onam', 'Lohri', 'All'];
...
const SellerDashboard = () => {
    ...
    const stats = [
        ...
        { label: 'Total Revenue', value: `₹${sellerOrders.reduce((s, o) => s + o.total, 0).toLocaleString()}`, icon: '💰', color: '#06D6A0' },
            ...
            {activeTab === 'overview' && (
                <div className="space-y-6">
                    ...
                    <div className="festival-card rounded-2xl p-5">
                        <h3 className="font-bold mb-4">Revenue (Last 12 Months)</h3>
                    ...
                    {sellerOrders.length === 0 ? <p className="text-center py-10 text-gray-500">No orders yet</p> : sellerOrders.map(order => (
                        <div key={order.id} className="festival-card rounded-xl p-4 mb-3">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <select value={order.status} onChange={e => { updateOrderStatus(order.id, e.target.value); showToast('Order status updated!'); }} className="text-xs border border-gray-200 rounded-lg px-2 py-1 outline-none">
                                    {['Processing', 'Confirmed', 'Shipped', 'Out for Delivery', 'Delivered'].map(s => <option key={s}>{s}</option>)}
                                </select>
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="festival-card rounded-2xl p-5">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={monthlyRevenue.slice(-6)}>
                                ...
                                <Tooltip />
                                <Line type="monotone" dataKey="orders" stroke="rgb(var(--color-primary))" strokeWidth={2} dot={{ fill: 'rgb(var(--color-primary))' }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
